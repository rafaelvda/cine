...
<html>
    ...
    <title>Ciné & Chill - Film recherché</title>
    ...
    //function search() {
        ...
        SELECT DISTINCT ?Couverture ?Titre ?Resume ?Realisateur ?Distribution 
        WHERE
            {
            ?film rdf:type dbo:Film;
                ...
                dbo:distributor ?Distribution;
                dbo:abstract ?Resume

                FILTER ( LANG ( ?Titre) = 'fr' )
                FILTER ( LANG ( ?Resume) = 'fr' )
                FILTER CONTAINS(?Titre,'mortel')
            ...
            Pour utiliser le mot tape par l'utilisateur on stocke cela dans une variables $refUser et on l'insere dans la requete
